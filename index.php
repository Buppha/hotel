...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  <div class="w3-container w3-row">
      ...
      <center><img src="<?php echo $objResult['img']?>" style="width:250px"></center>
  ...
  <div class="w3-bar-block">
    ...
    <a href="data_hotel.php" class="w3-bar-item w3-button w3-padding"><i class='fas fa-building'></i>  ข้อมูลโรงแรม</a>
    <a href="index.php" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-users fa-fw"></i>  เปิดห้อง</a>
     ...
     while($rows= mysqli_fetch_array($objQuery)) {
       ...
       <?php 
              if ($rows['status']=='1'){
                ...
                $name = $rows['name'];
                $ww = "SELECT MAX(booking_id) id 
                FROM tb_booking 
                WHERE hotel_d = '$hotel_id'
                AND room_no = '$name'";
                $objQueryww = mysqli_query($conn,$ww);
                $rowsww= mysqli_fetch_array($objQueryww);
           
                $booking_id = $rowsww['id'];
           ...
           $rows3= mysqli_fetch_array($qqq);
                ?>
               <font color="#ffffff">
                   <p> แบบ<?php echo $rows3['type_rent_name']?> ราคา <?php echo $rows3['total_cost']?> ฿</p>
                  <p> วันที่ <?php echo $rows3['date_come']?> เวลา <?php echo $rows3['time_come']?></p>
                </font>
                


                <div class="row">
                <form action="./update_Move.php">
                <div class="col-sm-6" >
                <p><input type="hidden" name="tyname" value="<?php echo $rows['ty_name']?>" ></p>
                    <p><input type="hidden" name="tybed" value="<?php echo $rows['ty_bed']?>" ></p>
                    <p><input type="hidden" name="name" value="<?php echo $rows['name']?>" ></p>
                <p> <center><button type="submit" class="btn btn-default btn-md">  ย้าย  </button></center></p>
                  </div>
                  </form>
                  <form action="./update_out2.php">

                  <div class="col-sm-6" >
                  <p><input type="hidden" name="name" value="<?php echo $rows['name']?>" ></p>
 

                <p> <center><button type="submit" class="btn btn-default btn-md" onclick="return confirm(' ยืนยันการออก')">  ออก  </button></center></p>
                  </div>
                      </form>
