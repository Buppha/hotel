...
<hr>
 ...
 <input type="text" class="form-control" id="name" name="name" required="required">
      ...
      <select class="form-control" id="ty_room" name="ty_room">
          ...
          $q = " SELECT * FROM  tb_typeroom
          WHERE hotelid = '$hotel_id'
          group by type_room,type_bed";
                                        
          $qmany = mysqli_query($conn,$q);
          while($row= mysqli_fetch_array($qmany)) { ?>

            <option value="<?php echo $row['typeroom_id']."|".$row['type_room']."|".$row['type_bed']?>" >
              <?php echo $row['type_room']." ". $row['type_bed'] ?>
            </option>
  ...
  <center>
		...
		<a href="#" class="btn btn-info" role="button">ห้องทั้งหมด</a>
		<a href="setting2.php" class="btn" role="button">ราคา</a>
    ...
    <table role="table">
      ...
      <thead role="rowgroup">
        ...
        <th role="columnheader">ลำดับ</th>
          <th role="columnheader">ชื่อห้อง</th>
          <th role="columnheader">ประเภทห้อง</th>
          <th role="columnheader">ประเภทเตียง</th>
          <th role="columnheader">สถานะตอนนี้</th>

          <th role="columnheader">แก้ไข</th>
          <th role="columnheader">ลบ</th>
      ...
      <tbody role="rowgroup">
         <?php
             include './connect.php';
        $hotel_id = $_SESSION["name_hotel"];
            $q = "SELECT tb_room.*,tb_hotel.*
            FROM tb_room,tb_hotel
            WHERE tb_room.hotel = tb_hotel.hotel_id
            AND tb_hotel.hotel_id = ' $hotel_id'
            ORDER BY tb_room.id ";
          ...
          <td><a href="edit_setting.php?id=<?php echo $row['id'] ?>"><i class="fa fa-edit" style="color:blue;"></i></a></td>

       
            <td> <a href="delete-setting.php?id=<?php echo $row['id'] ?>" onclick="return confirm(' ยืนยันการลบข้อมูล')"> <i  class="fa fa-close" style="color:red;" ></i></a></td>
